...
class TraceInteractionSerializer
  ...
  def detail
    summary.merge(
      instructions: llm_attribute("llm.instructions"),
      messages: messages,
    ...
    @messages ||= begin
      ...
      tool_spans.each do |span|
        ...
        stream << message(
          ...
          tool_calls: parsed(span.dig("attributes", "tool.arguments"))
        )
        stream << message(
          index += 1, role: "tool", content: span.dig("attributes", "tool.result"), at: finished,
          tool_name: name, tool_call_id: call_id
        )
      end

      if (completion = llm_attribute("llm.completion"))
  ...
  def generations
    llm_spans.map.with_index(1) do |span, index|
...

      {
        ...
        finish_reason: span["status"] == "ERROR" ? "error" : nil,
        tokens: {
