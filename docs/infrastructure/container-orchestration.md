...
- `ActionAgent::SandboxProvisionJob` - Background provisioning, in
...
`config/initializers/action_agent.rb` (`config.sandbox_backends`);
...
config/initializers/
  action_agent.rb              # registers the three backends with the engine

# In the actionagent gem (the dashboard engine, a separate gem):
#   ActionAgent::SandboxOrchestrator  # backend-agnostic orchestrator
#   ActionAgent::MockSandboxBackend   # in-memory backend it ships with
